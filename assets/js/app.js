...
const i18n = {
  en: {
    ...
    img_hint: "This photo can be pulled from public JSON or overridden from Admin.",
    skills: "Skills",
    projects: "Projects",
  ...
  ar: {
    ...
    img_hint: "يمكن جلب هذه الصورة من JSON العام أو تغييرها من لوحة الإدارة.",
    skills: "المهارات",
    projects: "المشاريع",
...
// ===== Prefs (theme/lang) =====
function loadPrefs(){ try { return JSON.parse(localStorage.getItem(LS.prefs) || '{}'); } catch(_){ return {}; } }
...
function initLang(){
  ...
  if(langBtn){
    langBtn.addEventListener('click', ()=>{
      ...
      // إعادة رسم المشاريع بنفس اللغة (لرسالة "لا توجد مشاريع")
      renderFromPublic();
    });
  }
}
...
function applyTheme(theme){
  ...
  if(themeBtn) themeBtn.setAttribute('data-i18n', 'theme'); // placeholder لو استخدمت ترجمة للزر لاحقاً
}
function initTheme(){
...
async function renderFromPublic(){
  ...
  data.projects.forEach(pj=>{
    ...
    card.innerHTML = `
      ...
      ${pj.tags ? `<div style="display:flex;flex-wrap:wrap;gap:6px;margin:6px 0">
        ${pj.tags.split(',').map(t=>`<span class='btn' style='padding:4px 8px;font-size:12px;border-radius:999px'>${t.trim()}</span>`).join('')}
      </div>`:''}
      ${pj.link ? `<a class="btn primary" href="${pj.link}" target="_blank" rel="noopener">View Details</a>` : `<button class="btn" disabled>View Details</button>`}
    `;
    grid.appendChild(card);
  });
}

// ===== Admin route (we already built standalone /admin) =====
function isAdminRoute(){
  return location.pathname.endsWith('/admin') || location.hash === '#admin';
}
